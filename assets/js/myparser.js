        ...
        for (var i in feilds) {
            ...
            if (type == "select-one") {
                ...
                function updateProgressBar(percentage) {
                    ...
                    const completeCircle = svg.querySelector('.complete');
                    const text = svg.querySelector('.percentage');

                    // Update data-percentage attribute
                    svg.dataset.percentage = percentage;

                    // Calculate stroke dash offset
                    const radius = parseInt(completeCircle.getAttribute('r'));
                    const circumference = 2 * Math.PI * radius;
                    const strokeDashOffset = circumference - ((percentage * circumference) / 100);

                    // Update stroke dash offset and text content
            ...
            if (id == "brks") {
                if (value == "1" || value == 1) {
                    $("#brk").html(
                        '<button class=" btn btn-success btn-sm" id="c_mainpg" type="button">' +
                        $("#brk").html() +
                        "</button>"
                    );
                } else {
                    $("#brk").html(
                        '<button class=" btn btn-danger btn-sm" id="c_mainpg" type="button">' +
                        $("#brk").html() +
                        "</button>"
                    );
                }
                ...
                if (value == "1" || value == 1) {
                    $("#hbm").html(
                        '<button class=" btn btn-success btn-sm" id="c_mainpg1" type="button"> Communicating </button>'
                    );
                } else {
                    $("#hbm").html(
                        '<button class=" btn btn-danger btn-sm" id="c_mainpg" type="button"> Not Communicating </button>'
                    );
                }
            } else if (id == "csm") {
                if (value == "1" || value == 1) {
                    $("#csm").html(
                        '<button class=" btn btn-success btn-sm" id="c_mainpg2" type="button"> Communicating </button>'
                    );
                } else {
                    $("#csm").html(
                        '<button class=" btn btn-danger btn-sm" id="c_mainpg3" type="button"> Not Communicating </button>'
                    );
                }
            } else if (id == "frstdt") {
...
function btnclick_actions(currelement, context, isconfirmationreq) {
    ...
    if (!(
        currpage == "diag.html" ||
        currpage == "dashboard.html" ||
        currpage == "login.html" ||
        currpage == "trends.html" ||
        currpage == "" ||
        firstload == true ||
        id == "loginbtn" ||
        id == "ct_p" || id == "sidebarToggle" ||
        ...
        if (r == true) {
            if (id == "facrst") {
                var res = prompt(
                    "All Settings will changed to default...To proceed please press 'F' to reset the device",
                    ""
                );
                if (res != "F") {
                    return false;
                ...
                var res = prompt(
                    "Current operation will Restart Gateway...To proceed please press 'R' to Restart",
                    ""
                );
                if (res != "R") {
                    return false;
                ...
                var res = prompt(
                    "All stored data will get Erased...To proceed please press 'Z' to zero fill the SD card",
                    ""
                );
                if (res != "Z") {
                    return false;
                ...
                var res = prompt(
                    "All Acknowledged Data will send again...To proceed please press 'A' to reset the server acknowledge",
                    ""
                );
                if (res != "A") {
                    return false;
...
function convertstringtoobj(datastring) {
    if (
        datastring != "" &&
        datastring != null &&
        datastring.indexOf("bid=") != -1 &&
        datastring.indexOf("popmsg=") != -1
    ) {
        var dataarray = datastring.split(splitter);
        if (datastring.indexOf(":lr=") != -1) {
            var lr =
                dataarray[searchStringInArray("lr=", dataarray)].split(valsplit)[1];
            if (lr == 1 || lr == "1") {
                window.location = "login.html";
            }
        }
        var bid =
            dataarray[searchStringInArray("bid=", dataarray)].split(valsplit)[1];
        var popmsg =
            dataarray[searchStringInArray("popmsg=", dataarray)].split(valsplit)[1];
        if (dataarray.length > 0) {
            ...
            for (var i in dataarray) {
                if (i <= dataarray.length) {
                    if (
                        dataarray[i].split(valsplit)[0] == "bid" ||
                        dataarray[i].split(valsplit)[0] == "popmsg" ||
                        dataarray[i].split(valsplit)[0] == "lr"
                    ) { } else {
    ...
    if (currpage == "cloudcfg.html") {
        ...
        for (var i = 1; i < 9; i++) {
            ...
            if (val > 1 && val111 == 1) {
                $("#dtd" + i)
                    .parent()
                    .prev()
                    .html("Demand TOD slot" + i);
                $("#t" + i + "hl")
                    .parent()
                    .prev()
                    .html("TOD slot " + i + " Step 2");
                $("#t" + i + "hl1")
                    .parent()
                    .prev()
                    .html("TOD slot " + i + " Step 3");
                $("#t" + i + "ll")
                    .parent()
                    .prev()
                    .html("TOD slot " + i + " Step 4");
                $("#t" + i + "hl1").attr(
                    "data-original-title",
                ...
                $("#t" + i + "hl")
                    .parent()
                    .prev()
                    .html("TOD slot " + i + " High Limit 1");
                $("#t" + i + "hl1")
                    .parent()
                    .prev()
                    .html("TOD slot " + i + " High Limit 2");
                $("#t" + i + "ll")
                    .parent()
                    .prev()
                    .html("TOD slot " + i + " Low Limit");
                $("#t" + i + "hl1").attr(
                    "data-original-title",
                    "1% to 99% of High limit 1"
                );
                $("#t" + i + "ll").attr(
                    "data-original-title",
                    "50% to 99% of High limit 2"
                );
            }
        }
        ...
        if (val1 == 0) {
            ...
            if ($("#rl1").parent(".material-switch").find("label").length > 0) { } else {
                ...
                $("#rl1")
                    ...
                    .append('<label for="rl1" class="label-success"></label>');
            }
        } else {
            ...
            if ($("#rl1").parent(".material-switch").find("label").length > 0) {
                ...
                $("#rl1")
                    .parent(".material-switch")
                    .append('<span class="btn btn-default btn-xs" >Disabled</label>');
            } else { }
        }
        if (val2 == 0) {
            ...
            if ($("#rl2").parent(".material-switch").find("label").length > 0) { } else {
                ...
                $("#rl2")
                    .parent(".material-switch")
                    .append('<label for="rl2" class="label-success"></label>');
            }
        } else {
            ...
            if ($("#rl2").parent(".material-switch").find("label").length > 0) {
                ...
                $("#rl2")
                    .parent(".material-switch")
                    .append('<span class="btn btn-default btn-xs" >Disabled</span>');
            } else { }
        }
        if (val3 == 0) {
            ...
            if ($("#rl3").parent(".material-switch").find("label").length > 0) { } else {
                ...
                $("#rl3")
                    .parent(".material-switch")
                    .append('<label for="rl3" class="label-success"></label>');
            }
        } else {
        ...
        if (val4 == 0) {
            ...
            if ($("#rl4").parent(".material-switch").find("label").length > 0) { } else {
                ...
                $("#rl4")
                    .parent(".material-switch")
                    .append('<label for="rl4" class="label-success"></label>');
            }
        } else {
